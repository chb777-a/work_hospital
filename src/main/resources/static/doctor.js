...
let showTable = (num) => {
    console.log(num);
    if(num == 1) {
        let registerTable = document.querySelector("#register_table");
        ...
        let updateTable = document.querySelector("#update_table");
        ...
        let cancelTable = document.querySelector("#cancel_table");
...
//환자 정보 전체 출력
let printAll = async () => {
    let tbody = document.querySelector("#doctor_table > tbody");
    ...
    try {
        ...
        response.data.forEach(doctor => {
            ...
            <tr>
                ...
                <td>
                    <button type="button" onclick="update(${doctor.doctorid}, '${doctor.name}', '${doctor.phone}', '${doctor.specialty}')">수정</button>
                    <button type="button" onclick="delete(${doctor.doctorid}, '${doctor.name}')">삭제</button>
                </td>
            </tr>
            `;
            select += `
...
// 환자 정보 등록
let register = async () => {
    let name = document.querySelector("#name_register");
    let specialty = document.querySelector("#specialty_register");
    let phone = document.querySelector("#phone_register");
    try {
...
// 환자 정보 수정
let update = async (doctorid, name, phone, specialty) => {
...
//환자 정보 삭제
let delete = async (doctorid, name) => {
